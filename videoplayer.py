...
class VideoPlayer:
    ...
    def BuildUI(self):
        ...
        self.player = mpv.MPV(wid=str(int(self.playerFrame.winfo_id(
        ...
        self.PlaybackButton = tk.Button(self.ControlFrame, text="play/pause",
                                        command=self.TogglePlayback).grid(column=2, row=0, sticky="nesw")
        self.NextButton = tk.Button(self.ControlFrame, text=">", command=self.Next).grid(
            column=3, row=0, sticky="nesw")
        self.PreviousButton = tk.Button(self.ControlFrame, text="<", command=self.Previous).grid(
            column=1, row=0, sticky="nesw")

        self.UrlEntryValue = tk.StringVar()
        self.UrlEntry = tk.Entry(self.ControlFrame, textvariable=self.UrlEntryValue).grid(
            column=4, row=1, sticky="nesw")
        self.AddToPlaylistButton = tk.Button(self.ControlFrame, text="Add", command=lambda: self.AddToPlaylist(
            self.UrlEntryValue.get())).grid(column=4, row=0, sticky="nesw")

        self.Volume = tk.DoubleVar(value=50)
        self.SetVolume(40)
        self.VolumeBar = tk.Scale(self.ControlFrame, from_=0, to=100, orient=tk.HORIZONTAL,
                                  variable=self.Volume, command=self.SetVolume).grid(column=3, row=1, sticky="nesw")

        # track bar
        self.PercentPos = tk.DoubleVar()
        self.TrackBar = tk.Scale(self.root, from_=0, to=100, orient=tk.HORIZONTAL,
                                 variable=self.PercentPos, command=self.SetPlaybackPos).grid(row=2, sticky="nesw")
    ...
    def ResumePlayback(self):
        ...
        if self.PLM.GetPercentPos() != 0:
            try:
                self.player._set_property(
                    "percent-pos", self.PLM.GetPercentPos())
                self.startupComplete = True
            except Exception as E:
                print("retry failed, trying again in one second reason: {}".format(E))
    ...
    def BuildHooks(self):
        ...
        self.player.observe_property(
            "percent-pos", handler=self.OnPositionChange)
    ...
    def UpdatePlaylistUI(self):
        ...
        with ytdl() as ydl:
            for url in self.PLM.GetList().GetPlaylist():
                ...
                frame = tk.Frame(
                    self.PlaylistFrame.scrollable_frame, background="black", border=10)
                frame.grid_columnconfigure(2, weight=1)
                tk.Button(frame, text="X", command=lambda URL=url: self.RemoveURL(
